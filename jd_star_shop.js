...
!(async () => {
  ...
  // console.log(`==================开始执行星店长任务==================`); for (let i = 0;
  // i < cookiesArr.length; i++) {   $.index = i + 1;   $.cookie = cookiesArr[i];
  //  $.isLogin = true;   $.nickName = '';   await TotalBean();   $.UserName =
  ...
  //
  //     if ($.isNode()) {       await notify.sendNotify(`${$.name}cookie已失效 -
  // ${$.UserName}`, `京东账号${$.index} ${$.UserName}\n请重新登录获取cookie`);     }
  // continue   }   await xdz(); } console.log(`开始执行星店长助力\n`);
  // if(cookiesArr.length > 1 && $.xdzHelpCodeList.length > 0){
  // if($.xdzHelpCodeList.length > 1){
  // $.xdzHelpCodeList.push($.xdzHelpCodeList.shift());   }   for (let i = 0; i <
  // cookiesArr.length; i++) {     $.cookie = cookiesArr[i];     $.UserName =
  // decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) &&
  // $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);     $.helpCode =
  // $.xdzHelpCodeList[i];     console.log(`${$.UserName},去助力${$.helpCode}`);
  // await help();     await $.wait(2000);     if($.xdzHelpCodeList[i+1]){
  // $.helpCode = $.xdzHelpCodeList[i+1];
  // console.log(`${$.UserName},去助力${$.helpCode}`);       await help();
  // await $.wait(2000);     }else{       $.helpCode = $.xdzHelpCodeList[0];
  // console.log(`${$.UserName},去助力${$.helpCode}`);       await help();
  // await $.wait(2000);     }   } }
  ...
  $
    .inviteCodeList
    .push(...getRandomArrayElements($.authorCodeList, 5));
  for (let i = 0; i < cookiesArr.length; i++) {
    $.cookie = cookiesArr[i];
    $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
...
async function xdz() {
  // $.xdzInfo = {}; await getXdzInfo(); if(JSON.stringify($.xdzInfo) === '{}'){
  // console.log(`获取活动数据为空`);   return ; } $.xdzUseInfo = {}; await
  // getXdzUseInfo(); if(JSON.stringify($.xdzUseInfo) === '{}'){
  // console.log(`获取用户数据为空`);   return ; } let tasksList =  $.xdzUseInfo.tasks;
  // for (let i = 0; i < tasksList.length; i++) {   $.oneTask = tasksList[i];
  // if($.oneTask.status !== 1){     continue;   }   if($.oneTask.taskType !==
  // '22' && $.oneTask.taskType !== '6'){
  // console.log(`执行任务：${$.oneTask.taskName}`);     let subItem =
  // $.oneTask.subItem;     for (let j = 0; j < subItem.length; j++) {
  // $.subItemInfo = subItem[j];       if(!$.subItemInfo.itemToken &&
  // $.subItemInfo.status !==1 ){         continue;       }       await
  // doXdzTask();       await $.wait(2000);     }   }else if($.oneTask.taskType
  // === '6'){     if($.oneTask.subItem && $.oneTask.subItem.length>0 &&
  // $.oneTask.times === 0){
  // $.xdzHelpCodeList.push($.oneTask.subItem[0].itemToken);
  // console.log(`助力码：${$.oneTask.subItem[0].itemToken}`);     }   } } let
  // awardVoList = $.xdzInfo.awardVoList; for (let i = 0; i < awardVoList.length;
  // i++) {   $.oneAwardInfo = awardVoList[i]; if($.oneAwardInfo.status === 1 &&
  // $.oneAwardInfo.grade === 1){   console.log(`执行抽奖`);   drawAward();   await
  // $.wait(2000); } }
...
async function guafen() {
  ...
  console.log(a);
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$
    .shopId}%22,%22nowTime%22:%22${a}%22,%22token%22:%22${$
      .tokenId}%22%7D&appid=xdz&functionId=mcxhd_starmall_getRedPacketAward&t=${Date
        .now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
...
async function help() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$
    .shopId}%22,%22itemToken%22:%22${$
      .helpCode}%22,%22token%22:%22${$
        .tokenId}%22%7D&appid=xdz&functionId=mcxhd_starmall_doTask&t=${Date
          .now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    'Origin': `https://h5.m.jd.com`,
...
async function drawAward() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$
    .shopId}%22,%22token%22:%22${$
      .tokenId}%22%7D&appid=xdz&functionId=mcxhd_starmall_drawAward&t=${Date
        .now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    'Origin': `https://h5.m.jd.com`,
...
async function getXdzUseInfo() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$
    .shopId}%22,%22token%22:%22${$
      .tokenId}%22%7D&appid=xdz&functionId=mcxhd_starmall_taskList&t=${Date
        .now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    'Origin': `https://h5.m.jd.com`,
...
async function getXdzInfo() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${$
    .shopId}%22,%22token%22:%22${$
      .tokenId}%22%7D&appid=xdz&functionId=mcxhd_starmall_getStarShopPage&t=${Date
        .now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    'Origin': `https://h5.m.jd.com`,
...
async function takePostRequest(type) {
  ...
  switch (type) {
    case 'activityStarBackGetProgressBarInfo':
      body = `functionId=activityStarBackGetProgressBarInfo&body={"starId":"${$
        .uniqueId}","linkId":"Y2aqxng42hZ0eGxGtbCMiQ"}&_t=${Date
          .now()}&appid=activities_platform`;
      myRequest = getPostRequest(body);
      break;
    case 'apTaskList':
      body = `functionId=apTaskList&body={"uniqueId":"${$
        .uniqueId}","linkId":"Y2aqxng42hZ0eGxGtbCMiQ"}&_t=${Date
          .now()}&appid=activities_platform`;
      myRequest = getPostRequest(body);
      break;
    case 'SIGN':
      body = `functionId=apDoTask&body={"taskType":"${$
        .oneTask
        .taskType}","taskId":${$
          .oneTask
          .id},"uniqueId":"${$
      ...
      break;
    case 'apTaskDetail':
      body = `functionId=apTaskDetail&body={"taskType":"${$
        .oneTask
        .taskType}","taskId":${$
          .oneTask
          .id},"uniqueId":"${$
            .uniqueId}","channel":4,"linkId":"Y2aqxng42hZ0eGxGtbCMiQ"}&_t=${Date
              .now()}&appid=activities_platform`;
      myRequest = getPostRequest(body);
      break;
    case 'apDoTask':
    ...
    case 'help':
      body = `functionId=activityStarBackGetProgressBarInfo&body={"starId":"${$
        .uniqueId}","sharePin":"${$
          .oneCode}","taskId":"129","linkId":"Y2aqxng42hZ0eGxGtbCMiQ"}&_t=${Date
            .now()}&appid=activities_platform`;
      myRequest = getPostRequest(body);
      break;
    case 'activityStarBackDrawPrize':
      body = `functionId=activityStarBackDrawPrize&body={"starId":"${$
        .uniqueId}","poolId":${$
          .onePrize
          .id},"pos":${$
            .onePrize
            .pos},"linkId":"Y2aqxng42hZ0eGxGtbCMiQ"}&_t=${Date
              .now()}&appid=activities_platform`;
      myRequest = getPostRequest(body);
      break;
    default:
