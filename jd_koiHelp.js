...
function submitCode(shareCode) {
    ...
    $.get({url: `http://www.jdhelp.cf/jdcodes/submit.php?code=${shareCode}&type=koi&user=${$.UserName}`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://www.jdhelp.cf/jdcodes/getcode.php?type=koi&num=20`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
