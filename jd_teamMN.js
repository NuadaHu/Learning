...
!(async () => {
    ...
    console.log(`若之前做过该活动，则无法重复入队。\n入口:\nhttps://lzkjdz-isv.isvjcloud.com/pool/captain/1410372?activityId=4e3b9b6233104c199c0c44ff6edbc85d&signUuid=22b6c048d84c481492c04d01f7f037e3`)
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = ['22b6c048d84c481492c04d01f7f037e3', 'c168745eb99145fdbc14c1a50ded1620', '']
            $.bean = 0;
            $.authorCode = ownCode ? ownCode : authorCodeList[random(0, authorCodeList.length)]
            $.activityId = '4e3b9b6233104c199c0c44ff6edbc85d'
            $.activityShopId = '1000014803'
            $.randomNum = random(1000000, 9999999)
            $.activityUrl = `https://lzkjdz-isv.isvjcloud.com/pool/captain/${$.randomNum}?activityId=${$.activityId}&signUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}`
            ...
            if ($.activityContent) {
                if ($.activityContent.canJoin) {
                    ...
                    if (!$.activityContent.openCard) {
                        ...
                        $.joinErrorTimes = 1;
                        await $.wait(1000)
                        await getShopOpenCardInfo({"venderId": "1000014803", "channel": 7005}, 1000014803)
                        $.errorJoinShop = '';
                        //await joinShop()
                        for (i = 0; i < 4; i++) {
                            await bindWithVender({"venderId": "1000014803", "shopId": "1000014803", "bindByVerifyCodeFlag": 1, "registerExtend": {}, "writeChildFlag": 0, "activityId": 2006823, "channel": 7005}, $.activityShopId)
                            if ($.errorJoinShop.indexOf('活动太火爆，请稍后再试') > -1) {
                                console.log(`第${$.joinErrorTimes}次 重新开卡`)
                                await $.wait(1000)
                                $.joinErrorTimes++
                            } else {
                                break;
...
function bindWithVender(body, venderId) {
    ...
    let opt = {
        'url': `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=bindWithVender&body=${encodeURIComponent(JSON.stringify(body))}&client=H5&clientVersion=9.2.0&uuid=88888&h5st=${h5st}`,
        'headers': {
            'Host': 'api.m.jd.com',
            'Accept': '*/*',
            'Connection': 'keep-alive',
            ...
            try {
                if (err) {
                    console.log(err)
                        ...
                        $.errorJoinShop = JSON.parse(data).message;
        ...
        $.get({
            'url': $.activityUrl, 'headers': {
                'user-agent': $.isNode() ? process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT :
                    require('./USER_AGENTS').USER_AGENT : $.getdata('JDUA') ? $.getdata('JDUA') : 'jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1'
            }
        }, (err, resp, data) => {
            try {
                refreshActivityCookie(resp)
...
function refreshActivityCookie(resp) {
    let LZ_TOKEN_KEY = '';
    let LZ_TOKEN_VALUE = '';
    let lz_jdpin_token = '';
    ...
    if (cookies) {
        if (typeof cookies != 'object') {
            cookies_re = cookies.split(',');
        } else cookies_re = cookies;
        for (let _0x1ecb99 of cookies_re) {
            let setCookie = _0x1ecb99.split(';')[0].trim();
            if (setCookie.split('=')[1]) {
                if (setCookie.indexOf('LZ_TOKEN_KEY=') > -1) {
                    LZ_TOKEN_KEY = setCookie.replace(/ /g, '') + ';';
                }
                if (setCookie.indexOf('LZ_TOKEN_VALUE=') > -1) {
                    LZ_TOKEN_VALUE = setCookie.replace(/ /g, '') + ';';
                }
                if (setCookie.indexOf('lz_jdpin_token=') > -1) {
                    lz_jdpin_token = ('' + setCookie.replace(/ /g, '')) + ';';
                }
            }
        }
    }
