...
function signPrizeDetailList() {
    return new Promise(async resolve => {
        let body = {"linkId":linkId,"serviceName":"dayDaySignGetRedEnvelopeSignService","business":1,"pageSize":20,"page":1};

        const options = {
            url: `https://api.m.jd.com/?functionId=signPrizeDetailList&body=${escape(JSON.stringify(body))}&_t=${+new Date()}&appid=activities_platform&clientVersion=3.5.0`,
            headers: {
                ...
                'Referer': `https://daily-redpacket.jd.com/?activityId=${linkId}&channel=wlfc&sid=a05ade6f8abfce24dbbc74fw&un_area=2`,
    ...
    return new Promise(resolve => {
        ...
        const options = {
            ...
            headers: {
                ...
                'Referer': `https://daily-redpacket.jd.com/?activityId=${linkId}&channel=wlfc5`,
                "Accept-Encoding": "gzip, deflate, br"
            }
        }
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 0) {
                            if (data.data.status === "310") {
                                console.log(`极速版签到提现成功！`)
                                allMessage += `京东账号${$.index} ${$.UserName}极速版签到提现成功！\n`;
                            } else {
                                console.log(`极速版签到提现：失败:${JSON.stringify(data)}\n`);
                            }
                        } else {
                            console.log(`极速版签到提现：异常:${JSON.stringify(data)}\n`);
                        }
                    }
                }
            } catch (e) {
