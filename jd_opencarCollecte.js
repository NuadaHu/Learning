/*
大牌集合 宠粉狂欢 [jd_opencarCollecte.js]
————————————————
...
(默认不加购 如需加购请设置环境变量[opencarCollecte_addSku]为"true"

默认脚本不执行
如需执行脚本请设置环境变量
opencarCollecte="true"
————————————————
因需要加载依赖文件
...
let opencar_addSku = "false"
opencar_addSku = $.isNode() ? (process.env.opencarCollecte_addSku ? process.env.opencarCollecte_addSku : `${opencar_addSku}`) : ($.getdata('opencarCollecte_addSku') ? $.getdata('opencarCollecte_addSku') : `${opencar_addSku}`);
let guaopencard = "false"
guaopencard = $.isNode() ? (process.env.opencarCollecte ? process.env.opencarCollecte : `${guaopencard}`) : ($.getdata('opencarCollecte') ? $.getdata('opencarCollecte') : `${guaopencard}`);
  ...
  try {
    ...
    for(let i in config){
      ...
      if($.task.memberTask && $.task.memberTask.memberList){
        let msg = ''
        if(guaopencard+"" != "true"){
          msg = '如需开卡请设置环境变量[opencarCollecte]为"true"'
        }
      ...
      if($.task.dailyTask && $.task.dailyTask.taskList){
        for (let t in $.task.dailyTask.taskList) {
          ...
          if(opencar_addSku+"" != "true" && 2 == $.oneTask.groupType) console.log('如需加购请设置环境变量[opencarCollecte_addSku]为"true"\n');
          if(opencar_addSku+"" != "true" && 2 == $.oneTask.groupType) continue;
