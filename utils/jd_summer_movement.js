...
class MovementFaker {
  ...
  static httpGet(url) {
    return new Promise((resolve, reject) => {
      const protocol = url.indexOf('http') !== 0
        ? 'https:'
        : '';
...
$.inviteList = ['HcmphLbwLg73eYHLF9Nm0uFhusphNoZddN2PHfKc_E6RIoyUqNqwAlJo4SEnJ4AClAzIyCACXbo3XL7I' +
  '0wxIsQ'];
$.byInviteList = [];
let uuid = 8888;
let cookiesArr = [];
...
!(async () => {
  ...
  let allCodeList = getRandomArrayElements([
    ...res,
    ...res2
  ], [
    ...res,
...
async function doTask() {
  ...
  for (let i = 0; i < $.taskList.length; i++) {
    ...
    if ([
      ...
      9,
      26
    ].includes($.oneTask.taskType) && $.oneTask.status === 1) {
  ...
  switch (type) {
    ...
    case 'olympicgames_receiveCash':
      if (data.code === 0 && data.data && data.data.result) {
        console.log('升级成功')
        // if(data.data.result.couponVO){   let res = data.data.result.couponVO
        // console.log(`获得[${res.couponName}]优惠券：${res.usageThreshold} 优惠：${res.quota}
        // 时间：${res.useTimeRange}`); }
      } else {
    ...
    case 'olympicgames_getTaskDetail':
      if (data.code === 0 && data.data.result) {
        ...
        if (data.data.result.inviteId) {
          $
            .inviteList
            .push({ 'ues': $.UserName, 'inviteId': data.data.result.inviteId, 'max': false });
        }
    ...
    case 'olypicgames_guradHome':
      ...
      if (data.data && data.data.result && data.data.bizCode === 0) {
        ...
        if (data.data.result.inviteId && Number(data.data.result.activityLeftSeconds) > 0) {
          $
            .byInviteList
            .push(data.data.result.inviteId)
  ...
  return new Promise((resolve) => {
    let url = {
      ...
      headers: {
        ...
        'Host': `api.m.jd.com`,
        'User-Agent': UA,
        // $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT :
        // (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ?
        // $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone;
        // CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko)
...
async function getPostRequest(body) {
  ...
  const headers = {
    ...
    'User-Agent': UA,
    // 'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ?
    // process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) :
    // ($.getdata('JDUA') ? $.getdata('JDUA') :
    // "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3
