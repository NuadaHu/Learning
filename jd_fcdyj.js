...
const notify = $.isNode()
	...
	: '';
let merge = {}
//let code = IOS等用户直接用NobyDa的jd cookie
let cookiesArr = [],
	cookie = '';
if ($.isNode()) {
	Object
...
!(async () => {
	...
	// if($.type == 2){ for (let i = 0 ; i <cookiesArr.length ; i++) {  cookie =
	// cookiesArr[i]; if (cookie) {  $.index = i + 1;
	// console.log(`\n******查询【京东账号${$.index}】红包情况\n`); await help($.rid,
	// $.inviter,$.helptype) } } }
...
function taskUrl(function_id, body) {
	return {
		...
		headers: {
			...
			"Host": "api.m.jd.com",
			"Referer": "https://618redpacket.jd.com/?activityId=DA4SkG7NXupA9sksI00L0g&channel=wjicon&si" +
				"d=0a1ec8fa2455796af69028f8410996aw&un_area=1_2803_2829_0",
			"Cookie": cookie,
			"User-Agent": $.isNode()
